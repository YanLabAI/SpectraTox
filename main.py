...
from modular_select import ML_Regress_auto, DL_Regress_auto, ML_Classifier_auto, DL_Classifier_auto
from modular_select import support
...
def train_IGC50():
    ## train ML
    train = np.array(pd.read_csv('./database/IGC50_train.csv', index_col=None).iloc[:,2:])
    train_tox = np.array(pd.read_csv('./database/IGC50_train.csv', index_col=None).iloc[:,1])
    test = np.array(pd.read_csv('./database/IGC50_test.csv', index_col=None).iloc[:,2:])
    test_tox = np.array(pd.read_csv('./database/IGC50_test.csv', index_col=None).iloc[:,1])

    data = np.concatenate((train, test), axis=0)
    len_train = len(train)
    train = StandardScaler().fit_transform(data)[:len_train,:]
    test = StandardScaler().fit_transform(data)[len_train:,:]
...
def train_Liver():
    train = np.array(pd.read_csv('./database/Liver_train.csv', index_col=None).iloc[:,2:])
    train_tox = np.array(pd.read_csv('./database/Liver_train.csv', index_col=None).iloc[:,1])
    test = np.array(pd.read_csv('./database/Liver_test.csv', index_col=None).iloc[:,2:])
    test_tox = np.array(pd.read_csv('./database/Liver_test.csv', index_col=None).iloc[:,1])
